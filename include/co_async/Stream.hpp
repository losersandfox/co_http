...
#include "co_async/StreamBase.hpp"
#include "co_async/stdio.hpp"

namespace co_async {

struct FileBuf {
    EpollLoop *mLoop = nullptr;
    ...
    FileBuf(EpollLoop &loop, AsyncFile &&file)
        : mLoop(&loop),
          mFile(std::move(file)) {}
    FileBuf(IOContext &ioContext, AsyncFile &&file)
        : mIOContext(&ioContext),
          mFile(std::move(file)) {}

    FileBuf() noexcept : mLoop(nullptr), mIOContext(nullptr) {}

    Task<std::size_t> read(std::span<char> buffer) {
        if(mIOContext) {
            auto n = co_await mIOContext->read_some(mFile.fileNo(), buffer.data(), buffer.size());
            co_return n;
        }
        co_return co_await read_file(*mLoop, mFile, buffer);
    }

    Task<std::size_t> write(std::span<char const> buffer) {
        if(mIOContext) {
            auto n = co_await mIOContext->write_some(mFile.fileNo(), buffer.data(), buffer.size());
            co_return n;
        }
        co_return co_await write_file(*mLoop, mFile, buffer);
    }
};
...
struct StdioBuf {
    ...
    StdioBuf(IOContext &ioContext, AsyncFile &&fileIn, AsyncFile &&fileOut)
        : mIOContext(&ioContext),
          mFileIn(std::move(fileIn)),
          mFileOut(std::move(fileOut)) {}

    StdioBuf() noexcept : mLoop(nullptr), mIOContext(nullptr) {}

    Task<std::size_t> read(std::span<char> buffer) {
        if(mIOContext) {
            auto n = co_await mIOContext->read_some(mFileIn.fileNo(), buffer.data(), buffer.size());
            co_return n;
        }
        co_return co_await read_file(*mLoop, mFileIn, buffer);
    }

    Task<std::size_t> write(std::span<char const> buffer) {
        if(mIOContext) {
            auto n = co_await mIOContext->write_some(mFileOut.fileNo(), buffer.data(), buffer.size());
            co_return n;
        }
        co_return co_await write_file(*mLoop, mFileOut, buffer);
    }
};
